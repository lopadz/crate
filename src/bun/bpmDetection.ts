...
function computeEnergyFrames(
  ...
  for (let start = 0; start + frameSamples <= samples.length; start += hopSamples) {
    ...
    frames.push(sum / frameSamples);
  }
  return frames;
}

function computeOnset(energyFrames: number[]): Float32Array {
...
function computeAutocorrelation(onset: Float32Array, lagMin: number, lagMax: number): Float32Array {
  const n = onset.length;
  const corr = new Float32Array(lagMax - lagMin + 1);
  for (let lag = lagMin; lag <= lagMax; lag++) {
    let sum = 0;
    const count = n - lag;
    for (let i = 0; i < count; i++) sum += onset[i] * onset[i + lag];
    corr[lag - lagMin] = sum / count;
  }
  return corr;
}
...
function applyOctaveCorrection(
  ...
  lagMin: number,
  lagMax: number,
  corrValues: Float32Array,
): number {
  const detectedBpm = (framesPerSec * 60) / bestLag;
  if (detectedBpm >= 105) return bestLag;
  ...
  return bestHalfLag >= 0 && bestHalfCorr > 0 ? bestHalfLag : bestLag;
}
...
export function detectBpm(samples: Float32Array, sampleRate: number): number | null {
  ...
  const energyFrames = computeEnergyFrames(samples, frameSamples, hopSamples);
  ...
  const onset = computeOnset(energyFrames);
  ...
  const corrValues = computeAutocorrelation(onset, lagMin, lagMax);

  let bestIdx = 0;
  for (let i = 1; i < corrValues.length; i++) {
    if (corrValues[i] > corrValues[bestIdx]) bestIdx = i;
  }
  if (corrValues[bestIdx] === 0) return null;

  // ── 4. Octave correction ──────────────────────────────────────────────────
  const bestLag = applyOctaveCorrection(bestIdx + lagMin, framesPerSec, lagMin, lagMax, corrValues);

  // ── 5. Convert lag to BPM ─────────────────────────────────────────────────
  return Math.round(((framesPerSec * 60) / bestLag) * 10) / 10;
}
